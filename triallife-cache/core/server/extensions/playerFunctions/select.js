...
async function selectCharacter(player, characterData) {
    ...
    alt.setTimeout(() => {
        if (player.data.pos)
            ...
            safe.setPosition(player, DEFAULT_CONFIG.PLAYER_NEW_SPAWN_POS.x, DEFAULT_CONFIG.PLAYER_NEW_SPAWN_POS.y, DEFAULT_CONFIG.PLAYER_NEW_SPAWN_POS.z);
        ...
        if (player.data.blood)
            safe.addBlood(player, player.data.blood);
        else
