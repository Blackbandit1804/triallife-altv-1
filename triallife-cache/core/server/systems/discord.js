...
export class DiscordController {
    static client = new Discord.Client({ ws: { intents: new Discord.Intents(Discord.Intents.ALL) } });
    ...
    static sendToChannel(channel_id, message) {
        if (!DiscordController.guild) {
            Logger.error(`You do not currently have a Discord Bot Setup for sending messages.`);
            return;
        }
        const channel = DiscordController.guild.channels.cache.find((x) => x.id === channel_id);
        if (!channel) {
