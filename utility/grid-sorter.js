...
const dataGroups = [
    { name: 'atm', url: `https://raw.githubusercontent.com/DurtyFree/gta-v-data-dumps/master/objectslocations/worldAtms.json` },
    { name: 'gas', url: `https://raw.githubusercontent.com/DurtyFree/gta-v-data-dumps/master/objectslocations/worldGasPumps.json` },
    { name: 'vending', url: `https://raw.githubusercontent.com/DurtyFree/gta-v-data-dumps/master/objectslocations/worldVendingMachines.json` },
    {
        name: 'clothing',
        items: [
...
function generateMinMaxs(division = 6) {
    ...
    for (let i = 0; i < division; i++) {
        const result = { maxY: maxY - (total / division) * i, minY: maxY - 2000 - (total / division) * i, objects: {} };
        groups.push(result);
    }
    return groups;
}
...
function sortListIntoGroups(name, items) {
    for (let i = 0; i < items.length; i++) {
        ...
        if (itemData.position) itemData.position = Object.fromEntries(Object.entries(itemData.position).map(([k, v]) => [k.toLowerCase(), v]));
        if (itemData.rotation) itemData.rotation = Object.fromEntries(Object.entries(itemData.rotation).map(([k, v]) => [k.toLowerCase(), v]));
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        if (index <= -1) continue;
        if (!currentGroups[index].objects[name]) currentGroups[index].objects[name] = [];
...
(async () => {
    ...
    for (let i = 0; i < dataGroups.length; i++) {
        ...
        const result = await axios.get(url).catch((err) => null);
        if (!result) {
