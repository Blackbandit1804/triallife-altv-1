...
alt.onServer(ViewCreator.Sync, handleSync);
alt.onServer(ViewCreator.Show, handleView);
alt.onServer(ViewCreator.AwaitModel, handleFinishSync);
alt.onServer(ViewCreator.AwaitName, handleNameFinish);
...
async function handleDone(newData, infoData, name: string) {
    await handleClose();
    alt.emitServer(ViewCreator.Done, newData, infoData, name);
}

async function handleCancel() {
    handleClose();
    alt.emitServer(ViewCreator.Done, oldCharacterData);
}
...
function handleCheckName(name: string): void {
    alt.emitServer(ViewCreator.AwaitName, name);
}
...
export async function handleSync(data: Partial<Design>): Promise<void> {
    ...
    if (modelNeeded !== native.getEntityModel(alt.Player.local.scriptID)) {
        ...
        alt.emitServer(ViewCreator.AwaitModel, data.sex);
    } else {
        handleFinishSync();
    }
}
