...
const ownershipBehavior = Behavior.CONSUMES_FUEL | Behavior.NEED_KEY_TO_START;
const tmpBehavior = Behavior.NO_KEY_TO_LOCK | Behavior.NO_KEY_TO_START | Behavior.UNLIMITED_FUEL | Behavior.NO_SAVE;
...
function despawn(id: number, player: alt.Player = null): boolean {
    ...
    if (vehicle.valid && vehicle.destroy) {
        alt.emit(TlrpVehicleEvent.DESPAWNED, vehicle);
        vehicle.destroy();
    }
...
function tempVehicle(player: alt.Player, model: string, pos: alt.IVector3, rot: alt.IVector3): alt.Vehicle {
    ...
    vehicle.setStreamSyncedMeta(VehicleDoorState.OWNER, vehicle.player_id);
    return vehicle;
}
...
function spawn(player: alt.Player, data: Vehicle): alt.Vehicle {
    ...
    vehicle.setStreamSyncedMeta(VehicleDoorState.OWNER, vehicle.player_id);
    alt.emit(TlrpVehicleEvent.SPAWNED, vehicle);
    return vehicle;
}
