...
import { EquipType } from '../../../shared/enums/equipType';
...
function isEquipmentSlotFree(p: alt.Player, slot: EquipType): boolean {
    if (slot >= 11) {
        return false;
    }
...
function equipmentRemove(p: alt.Player, slot: EquipType): boolean {
    if (slot >= 11) {
        return false;
    }
...
function isEquipmentSlotValid(item: Item, slot: EquipType) {
    if (slot >= 11) {
        return false;
    }
...
function equipmentAdd(p: alt.Player, item: Item, slot: EquipType): boolean {
    if (slot >= 11) {
        return false;
    }
...
function allItemRulesValid(
    ...
    if (endSlot) {
        ...
        if (!isFlagEnabled(item.behavior, ItemType.CAN_DROP) && endSlot.name === InvType.GROUND) {
            return false;
        }

        // Not equipment but going into equipment.
        if (!isFlagEnabled(item.behavior, ItemType.IS_EQUIPMENT) && endSlot.name === InvType.EQUIPMENT) {
            return false;
        }

        // Not a toolbar item but going into toolbar.
        if (!isFlagEnabled(item.behavior, ItemType.IS_TOOLBAR) && endSlot.name === InvType.TOOLBAR) {
            return false;
        }
