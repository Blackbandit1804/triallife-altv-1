...
function handleFuel(player: alt.Player, pos: alt.IVector3) {
    ...
    if (isFlagEnabled(lastVehicle.behavior, Behavior.UNLIMITED_FUEL)) {
    ...
    if (maximumTime < 3000) maximumTime = 3000;
...
function handleFinishFuel(player: alt.Player, fuelStatus: FuelStatus) {
    ...
    if (fuelStatus.timeout) alt.clearTimeout(fuelStatus.timeout);
    if (player.data.cash < fuelStatus.maxCost) {
        playerFuncs.emit.notification(player, LocaleController.get(LOCALE_KEYS.FUEL_CANNOT_AFFORD));
        return;
    }
    if (Date.now() >= fuelStatus.endTime) {
        playerFuncs.economy.sub(player, EconomyType.CASH, fuelStatus.maxCost);
        ...
        if (owner) vehicleFuncs.save.data(owner, fuelStatus.vehicle);
        playerFuncs.emit.notification(
            player,
    ...
    playerFuncs.economy.sub(player, EconomyType.CASH, totalCost);
    fuelStatus.vehicle.fuel += totalFuel;
    fuelStatus.vehicle.data.fuel += totalFuel;
    if (fuelStatus.vehicle.fuel > 100) fuelStatus.vehicle.fuel = 100;
    if (fuelStatus.vehicle.data.fuel > 100) fuelStatus.vehicle.data.fuel = 100;
    vehicleFuncs.setter.updateFuel(fuelStatus.vehicle);
    fuelStatus.vehicle.fuel = fuelStatus.vehicle.data.fuel;
    fuelStatus.vehicle.setStreamSyncedMeta(VehicleDoorState.FUEL, fuelStatus.vehicle.data.fuel);
    const owner = alt.Player.all.find((p) => p.valid && p.id === fuelStatus.vehicle.player_id);
    if (owner) vehicleFuncs.save.data(owner, fuelStatus.vehicle);
    playerFuncs.emit.notification(
        player,
