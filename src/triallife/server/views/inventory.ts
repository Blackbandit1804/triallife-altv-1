...
import { InvType } from '../../shared/enums/invType';
import { ItemType } from '../../shared/enums/itemType';
import { SystemEvent } from '../../shared/enums/system';
import { ViewInventory } from '../../shared/enums/views';
...
export class InventoryController {
    ...
    static processItemMovement(
        ...
        if (selectData.name === InvType.TOOLBAR && endData.name !== InvType.TOOLBAR) {
            player.removeAllWeapons();
        }

        // Handle Drop Ground
        if (endData.name === InvType.GROUND) {
            InventoryController.handleDropGround(player, selectedSlot, tab);
            return;
        }

        // Pickup Item from Ground
        if (selectData.name === InvType.GROUND) {
    ...
    static handleDropGround(player: alt.Player, selectedSlot: string, tab: number) {
        ...
        playerFuncs.emit.animation(player, 'random@mugging4', 'pickup_low', 33, 1200);
        alt.emit(TlrpPlayerEvent.DROPPED_ITEM, player, itemClone);
    }
    ...
    static handlePickupGround(
        ...
        const isGoingToEquipment = endData.name === InvType.EQUIPMENT;
...
alt.onClient(ViewInventory.Use, InventoryController.processUse);
alt.onClient(ViewInventory.Process, InventoryController.processItemMovement);
alt.onClient(ViewInventory.Split, InventoryController.processSplit);
alt.onClient(ViewInventory.Pickup, InventoryController.handleProcessPickup);
