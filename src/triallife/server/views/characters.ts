...
import { ViewCharacters, ViewCreator } from '../../shared/enums/views';
...
alt.onClient(ViewCharacters.Select, handleSelectCharacter);
alt.onClient(ViewCharacters.New, handleNewCharacter);
alt.onClient(ViewCharacters.Delete, handleDelete);
...
export async function goToCharacterSelect(player: Player): Promise<void> {
    ...
    alt.setTimeout(() => {
        alt.emitClient(player, ViewCharacters.Show, characters);
    }, 1000);
}
...
export async function handleSelectCharacter(player: Player, id: string): Promise<void> {
    ...
    alt.emitClient(player, ViewCharacters.Done);
...
export function handleNewCharacter(player: Player): void {
    ...
    alt.setTimeout(() => {
        alt.emitClient(player, ViewCreator.Show, null, true, false, totalCharacters); // _oldCharacterData, _noDiscard, _noName
    }, 1000);
}
