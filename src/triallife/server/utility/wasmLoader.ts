...
function convert(v1: any, v2: any) {
    ...
    if (process.platform.includes('win')) data = data.replace(/\\/g, '/');
    alt.emit(helpers.__getString(v2), data);
}

export class WASM {
    static imports = { 'ex.emit': alt.emit, 'ex.virtualImport': convert };
    ...
    static async load<T>(buffer: Buffer = null): Promise<T | null> {
        const { exports }: any | null = await loader.instantiate(buffer, { index: WASM.imports }).catch((err) => {
            console.error(err);
            return null;
        });
