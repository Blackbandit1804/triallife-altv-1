...
import { SYSTEM_EVENTS, ViewEventsInventory, InventoryTypes, ItemTypes } from '../../shared/utility/enums';
...
export class InventoryController {
    ...
    static processItemMovement(player: alt.Player, selectedSlot: string, endSlot: string, tab: number, hash: string | null): void {
        ...
        if (selectData.name === InventoryTypes.TOOLBAR && endData.name !== InventoryTypes.TOOLBAR) {
            player.removeAllWeapons();
        }
        if (endData.name === InventoryTypes.GROUND) {
            InventoryController.handleDropGround(player, selectedSlot, tab);
            return;
        }
        if (selectData.name === InventoryTypes.GROUND) {
        ...
        if (endData.name === InventoryTypes.TAB) {
        ...
        const isEquipmentItem = isFlagEnabled(itemClone.behavior, ItemTypes.IS_EQUIPMENT);
    ...
    static handleDropGround(player: alt.Player, selectedSlot: string, tab: number) {
        ...
        if (!isFlagEnabled(itemClone.behavior, ItemTypes.CAN_DROP)) {
            playerFuncs.sync.inventory(player);
            return;
        }
        ...
        if (isFlagEnabled(itemClone.behavior, ItemTypes.DESTROY_ON_DROP)) {
    ...
    static handlePickupGround(player: alt.Player, endData: CategoryData, endSlotIndex: number, hash: string | null, tab: number) {
        ...
        const isEquipmentItem = isFlagEnabled(droppedItem.item.behavior, ItemTypes.IS_EQUIPMENT);
        const isGoingToEquipment = endData.name === InventoryTypes.EQUIPMENT;
    ...
    static processUse(player: alt.Player, selectedSlot: string, tab: number) {
        ...
        if (!isFlagEnabled(item.behavior, ItemTypes.CONSUMABLE)) {
            playerFuncs.sync.inventory(player);
            return;
        }
        if (!isFlagEnabled(item.behavior, ItemTypes.SKIP_CONSUMABLE)) {
            item.quantity -= 1;
            if (item.quantity <= 0) playerFuncs.inventory.inventoryRemove(player, slot, tab);
            else playerFuncs.inventory.replaceInventoryItem(player, item, tab);
            playerFuncs.save.field(player, 'inventory', player.data.inventory);
            playerFuncs.sync.inventory(player);
        }
...
alt.onClient(ViewEventsInventory.Use, InventoryController.processUse);
alt.onClient(ViewEventsInventory.Process, InventoryController.processItemMovement);
alt.onClient(ViewEventsInventory.Split, InventoryController.processSplit);
alt.onClient(ViewEventsInventory.Pickup, InventoryController.handleProcessPickup);
