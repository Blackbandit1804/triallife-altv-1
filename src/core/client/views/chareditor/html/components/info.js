...
const InfoComponent = Vue.component('tab-info', {
    ...
    methods: {
        ...
        verifyName(newValue) {
            ...
            alt.emit('chareditor:CheckName', newValue);
            alt.emit('chareditor:DisableControls', false);
        },
        ...
        disableControls() {
            if (!('alt' in window)) return;
            alt.emit('chareditor:DisableControls', true);
        },
        ...
        save() {
            ...
            if (!('alt' in window)) return;
            alt.emit('chareditor:Done', this.data, this.infodata, this.infodata.name);
        }
    },
    watch: {
        day(newValue) {
    ...
    mounted() {
        this.nameRules = [
            ...
            (v) => (v && v.length <= 16) || `${this.locales.LABEL_NAME} ${this.locales.LABEL_CANNOT_EXCEED} 16 ${this.locales.LABEL_CHARACTER}`,
            (v) => (v && v.length >= 5) || `${this.locales.LABEL_NAME} ${this.locales.LABEL_CANNOT_BE_LESS} 5 ${this.locales.LABEL_CHARACTER}`,
        ...
        this.yearRules = [
            (v) => !!v || this.locales.LABEL_FIELD_REQUIRED,
            (v) => (v && v !== ``) || this.locales.LABEL_FIELD_REQUIRED,
            (v) => (v && v >= 1940) || `${this.locales.LABEL_YEAR} ${this.locales.LABEL_CANNOT_BE_LESS} ${this.minYear}`,
        ...
        if (!(`alt` in window)) return;
        alt.on('chareditor:IsNameAvailable', this.handleNameAvailable);
    },
    template: `
        <template>
            <div class="wrapper flex-grow-1">
                <div class="stack">
                    <div class="overline grey--text">{{ locales.LABEL_NAME }}</div>
                    ...
                    <div class="split">
                        ...
                        <template v-else>
                            <v-icon class="spinner green--text text--lighten-2">icon-spinner</v-icon>
                            <span class="mr-3"></span>
                        </template>
                        <v-text-field 
                            :rules="nameRules" 
                            type="name" 
                            placeholder="Jonny_Walker" 
                ...
                <div class="stack">
                    <div class="overline grey--text">{{ locales.LABEL_BIRTHDAY }}</div>
                    <div class="stack">
                        <div class="split mb-4">
                            <template v-if="dayValid && monthValid && yearValid">
                                <v-icon class="green--text mr-3 pb-1">icon-checkmark</v-icon>
                            </template>
                            <template v-else>
                                <v-icon class="red--text mr-3 pb-1">icon-error</v-icon>
                ...
                <div class="stack">
                    <div class="overline grey--text">{{ locales.LABEL_GENDER }}</div>
                    <div class="split">
