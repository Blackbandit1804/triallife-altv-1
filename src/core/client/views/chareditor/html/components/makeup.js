const MakeupComponent = Vue.component('makeup', {
    ...
    methods: {
        ...
        decrementParameter(index, propertyName, min, max, incrementValue) {
            ...
            if (currentValues[index][propertyName] < min) currentValues[index][propertyName] = max;
        ...
        incrementParameter(index, propertyName, min, max, incrementValue) {
            const currentValues = [...this.data.colorOverlays];
            currentValues[index][propertyName] += incrementValue;
            if (currentValues[index][propertyName] > max) currentValues[index][propertyName] = min;
            this.data.colorOverlays = currentValues;
            this.$root.$emit('updateCharacter');
        },
        ...
        getLocale(id) {
            return this.locales.makeupComponent.ids[id] ? this.locales.makeupComponent.ids[id] : `COULD NOT FIND LOCALE FOR makeupComponent.${id}`;
        }
    },
    template: `
        <template>
            <div class="wrapper flex-grow-1">
                <template v-for="(option, i) in MakeupList" :key="i">
                    ...
                    <div class="button-group pa-2">
                        <div class="overline grey--text">{{ locales.makeupComponent.LABEL_OPACITY }}</div>
                        <div class="split mt-4 mb-4">
                            <v-chip class="green--text mr-3" label outlined>{{ data.colorOverlays[i].opacity }}</v-chip>
                        ...
                        <div class="overline grey--text">{{ locales.makeupComponent.LABEL_STYLE }}</div>
                        ...
                        <div class="split mt-4 mb-4">
                            <v-chip class="green--text mr-3" label outlined>{{ data.colorOverlays[i].value }}</v-chip>
                        ...
                        <div class="overline grey--text">{{ locales.makeupComponent.LABEL_COLOUR1 }}</div>
                        <div class="split flex-grow-1">
                        ...
                        <div class="split mt-4 mb-4">
                            <v-chip class="green--text mr-3" label outlined>{{ data.colorOverlays[i].color1 }}</v-chip>
                        ...
                        <template v-if="option.color2 !== undefined">
                            <div class="overline grey--text">{{ locales.makeupComponent.LABEL_COLOUR2 }}</div>
                            <div class="split flex-grow-1">
                            ...
                            <div class="split mt-4 mb-4">
                                <v-chip class="green--text mr-3" label outlined>{{ data.colorOverlays[i].color2 }}</v-chip>
