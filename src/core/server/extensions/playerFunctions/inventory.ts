...
function allItemRulesValid(player: alt.Player, item: Item, endSlot: CategoryData, endSlotIndex: number | null, customItemRules: Array<Function>, tab: number | null): boolean {
    ...
    if (endSlot) {
        ...
        if (isFlagEnabled(item.behavior, ItemType.IS_EQUIPMENT) && endSlot.name === InventoryType.EQUIPMENT) if (!isEquipmentSlotValid(item, endSlotIndex)) return false;
    }
    if (customItemRules.length >= 1) {
        for (let i = 0; i < customItemRules.length; i++) {
            if (!customItemRules[i](player, item, endSlot ? endSlot.name : null, endSlotIndex, tab)) return false;
        }
    }
    return true;
}
