...
import { Vehicle_Behavior, Vehicle_Door_List, Vehicle_Lock_State, Vehicle_State } from '../../../shared/utility/enums';
...
function lock(v: alt.Vehicle, player: alt.Player, lockState: Vehicle_Lock_State): boolean {
    if (!isFlagEnabled(v.behavior, Vehicle_Behavior.NO_KEY_TO_LOCK)) if (!getter.isOwner(v, player) && !keys.has(v, player)) return false;
    v.lockStatus = lockState;
    v.setStreamSyncedMeta(Vehicle_State.LOCK_STATE, v.lockStatus);
    if (v.lockStatus === Vehicle_Lock_State.LOCKED) for (let i = 0; i < 6; i++) doorOpen(v, player, i, false);
    return true;
}
...
function doorOpen(v: alt.Vehicle, player: alt.Player, index: Vehicle_Door_List, state: boolean, bypass: boolean = false): void {
    if (!isFlagEnabled(v.bodyHealth, Vehicle_Behavior.NO_KEY_TO_LOCK) && !bypass)
        if (!getter.isOwner(v, player) && !keys.has(v, player) && getter.lockState(v) !== Vehicle_Lock_State.UNLOCKED) return;
...
function updateFuel(v: alt.Vehicle) {
    ...
    v.fuel = tlrp.Math.sub(v.fuel, DEFAULT_CONFIG.FUEL_LOSS_PER_PLAYER_TICK);
