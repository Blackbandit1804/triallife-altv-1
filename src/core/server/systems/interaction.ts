...
export class InteractionController {
        ...
        gridData.forEach((grid) => {
            Object.keys(grid.objects).forEach((key) => {
                ...
                infoData.forEach((info) => {
                    ...
                    if (!InteractionController.Interactions[category]) InteractionController.Interactions[category] = [];
                    count += 1;
    ...
    static addInteraction(identifierAndEventName: string, position: alt.IVector3, range: number, activationText: string, blip: Blip, isServerEvent: boolean) {
        ...
        if (!InteractionController.Interactions[identifierAndEventName]) InteractionController.Interactions[identifierAndEventName] = [];
    ...
    static handleInteraction(player: alt.Player, type: string) {
        ...
        const closestInteraction = InteractionController.Interactions[type].find((interaction) => {
            if (distance2d(interaction.pos, player.pos) <= DEFAULT_CONFIG.MAX_INTERACTION_DISTANCE) return true;
            return false;
        });
